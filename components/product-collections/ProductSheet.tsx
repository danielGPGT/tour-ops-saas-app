...
import { createProduct, updateProduct } from "@/app/product-collections/actions";
...
export function ProductSheet({ isOpen, onClose, product, productTypes = [] }: ProductSheetProps) {
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    ...
    if (isEditing && product) {
      await updateProduct(product.id, {
        ...values,
        type: values.type as "accommodation" | "activity" | "event" | "transfer" | "package",
  ...
  const selectedType = productTypes.find(t => t.name === initialValues.type);
  ...
  return (
    ...
    >
      {({ values, set }) => {
        const currentSelectedType = productTypes.find(t => t.name === values.type);
            ...
            {currentSelectedType && (
              <div className="p-3 bg-muted/50 rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-medium text-sm">{currentSelectedType.name}</span>
                </div>
