...
export function ProductManagementClient({ product, variants, suppliers }: ProductManagementClientProps) {
  ...
  const variantColumns: DataTableColumn<Variant>[] = [
    {
      key: "name",
      header: "Variant Name",
      width: "w-[250px]",
  ...
  const variantBulkActions = [
    {
      ...
      onClick: () => {
        ...
        toast.success(`Activated ${selectedVariants.length} variants`);
        setSelectedVariants([]);
      }
    },
    {
      ...
      onClick: () => {
        ...
        toast.success(`Deactivated ${selectedVariants.length} variants`);
        setSelectedVariants([]);
      }
    },
    {
      ...
      onClick: () => {
        if (confirm(`Are you sure you want to delete ${selectedVariants.length} variants?`)) {
          // TODO: Implement bulk delete
          toast.success(`Deleted ${selectedVariants.length} variants`);
          setSelectedVariants([]);
        }
      },
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground capitalize">
                {product.type} • {variantsData.length} variants
              </p>
            </div>
          </div>
        </div>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList>
          ...
          <TabsTrigger value="variants">Variants ({variantsData.length})</TabsTrigger>
            ...
            cards={[
              {
                ...
                description: `${variantsData.length} variant${variantsData.length !== 1 ? 's' : ''} configured`
              },
              {
                id: "active-variants",
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Manage all variants for {product.name}
              </p>
            </div>
            <Button
              onClick={() => {
        ...
        <Dialog open={showVariantWizard} onOpenChange={setShowVariantWizard}>
          <DialogContent className="!max-w-4xl max-h-[90vh] overflow-y-auto">
            <MultiRatePlanWizard
              ...
              onComplete={(data) => {
                console.log('Multi-rate wizard completed:', data);
                setShowVariantWizard(false);
