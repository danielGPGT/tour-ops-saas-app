...
CREATE TABLE public.contract_cancellation_policies (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  contract_id uuid NOT NULL,
  days_before_from integer,
  days_before_to integer,
  penalty_type character varying NOT NULL CHECK (penalty_type::text = ANY (ARRAY['none'::character varying, 'percentage'::character varying, 'fixed_amount'::character varying, 'forfeit_deposit'::character varying, 'forfeit_all'::character varying]::text[])),
  penalty_percentage numeric CHECK (penalty_percentage >= 0::numeric AND penalty_percentage <= 100::numeric),
  penalty_amount numeric,
  description text,
  applies_to character varying DEFAULT 'all'::character varying CHECK (applies_to::text = ANY (ARRAY['all'::character varying, 'deposit_only'::character varying, 'balance_only'::character varying]::text[])),
  sort_order integer DEFAULT 0,
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT contract_cancellation_policies_pkey PRIMARY KEY (id),
  CONSTRAINT contract_cancellation_policies_contract_id_fkey FOREIGN KEY (contract_id) REFERENCES public.contracts(id)
);
CREATE TABLE public.contract_commission_tiers (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  contract_id uuid NOT NULL,
  revenue_from numeric NOT NULL,
  revenue_to numeric,
  commission_rate numeric NOT NULL CHECK (commission_rate >= 0::numeric AND commission_rate <= 100::numeric),
  sort_order integer DEFAULT 0,
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT contract_commission_tiers_pkey PRIMARY KEY (id),
  CONSTRAINT contract_commission_tiers_contract_id_fkey FOREIGN KEY (contract_id) REFERENCES public.contracts(id)
);
CREATE TABLE public.contract_deadlines (
  ...
  calculate_from character varying CHECK (calculate_from::text = ANY (ARRAY['arrival'::character varying, 'departure'::character varying, 'booking_date'::character varying]::text[])),
  penalty_type character varying CHECK (penalty_type::text = ANY (ARRAY['none'::character varying, 'percentage'::character varying, 'fixed_amount'::character varying, 'forfeit_deposit'::character varying, 'forfeit_all'::character varying]::text[])),
  penalty_value numeric,
  penalty_description text,
  status character varying DEFAULT 'pending'::character varying CHECK (status::text = ANY (ARRAY['pending'::character varying, 'met'::character varying, 'missed'::character varying, 'waived'::character varying, 'not_applicable'::character varying]::text[])),
...
CREATE TABLE public.contract_payment_schedules (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  contract_id uuid NOT NULL,
  payment_stage character varying NOT NULL,
  payment_type character varying CHECK (payment_type::text = ANY (ARRAY['percentage'::character varying, 'fixed_amount'::character varying]::text[])),
  percentage numeric CHECK (percentage >= 0::numeric AND percentage <= 100::numeric),
  fixed_amount numeric,
  due_type character varying CHECK (due_type::text = ANY (ARRAY['days_before_arrival'::character varying, 'days_after_booking'::character varying, 'fixed_date'::character varying]::text[])),
  days_before integer,
  days_after integer,
  due_date date,
  description text,
  ...
  CONSTRAINT contract_payment_schedules_contract_id_fkey FOREIGN KEY (contract_id) REFERENCES public.contracts(id)
);
CREATE TABLE public.contracts (
  ...
  contract_type character varying NOT NULL DEFAULT 'net_rate'::character varying CHECK (contract_type::text = ANY (ARRAY['net_rate'::character varying, 'commissionable'::character varying, 'allocation'::character varying, 'on_request'::character varying]::text[])),
  valid_from date NOT NULL,
  valid_to date NOT NULL,
  signed_date date,
  currency character varying DEFAULT 'USD'::character varying,
  commission_rate numeric CHECK (commission_rate >= 0::numeric AND commission_rate <= 100::numeric),
  commission_type character varying DEFAULT 'percentage'::character varying CHECK (commission_type::text = ANY (ARRAY['percentage'::character varying, 'fixed_amount'::character varying, 'tiered'::character varying, 'none'::character varying]::text[])),
  booking_cutoff_days integer CHECK (booking_cutoff_days > 0),
  signed_document_url text,
  contract_files jsonb DEFAULT '[]'::jsonb,
  terms_and_conditions text,
  special_terms text,
  notes text,
  status character varying DEFAULT 'active'::character varying CHECK (status::text = ANY (ARRAY['draft'::character varying, 'active'::character varying, 'expired'::character varying, 'terminated'::character varying, 'suspended'::character varying]::text[])),
  created_by uuid,
  updated_by uuid,
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
